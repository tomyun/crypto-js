YUI.add('algo-sha256-test', function (Y) {
    ...
    Y.Test.Runner.add(new Y.Test.Case({
        ...
        testVector1: function () {
            var actual = C.algo.SHA256.create().compute('');

            Y.Assert.areEqual('e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855', actual);
        },

        testVector2: function () {
            var actual = C.algo.SHA256.create().compute('a');

            Y.Assert.areEqual('ca978112ca1bbdcafac231b39a23dc4da786eff8147c4e72b9807785afee48bb', actual);
        },

        testVector3: function () {
            var actual = C.algo.SHA256.create().compute('abc');

            Y.Assert.areEqual('ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad', actual);
        },

        testVector4: function () {
            var actual = C.algo.SHA256.create().compute('message digest');

            Y.Assert.areEqual('f7846f55cf23e14eebeab5b4e1550cad5b509e3348fbc4efa3a1413d393cb650', actual);
        },

        testVector5: function () {
            var actual = C.algo.SHA256.create().compute('abcdefghijklmnopqrstuvwxyz');

            Y.Assert.areEqual('71c480df93d6ae2f1efad1447c66c9525e316218cf51fc8d9ed832f2daf18b73', actual);
        },

        testVector6: function () {
            var actual = C.algo.SHA256.create().compute('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789');

            Y.Assert.areEqual('db4bfcbd4da0cd85a60c3c37d3fbd8805c77f15fc6b1fdfe614ee0a7c8fdb4c0', actual);
        },

        testVector7: function () {
            var actual = C.algo.SHA256.create().compute('12345678901234567890123456789012345678901234567890123456789012345678901234567890');
